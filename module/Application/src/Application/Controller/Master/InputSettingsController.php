...
class InputSetListsController extends ControllerBasicWithAuthentication
{
    protected $formName = 'InputSetListForm';

    public $inputSetList;

    public function getInputSetList(){
        if($this->inputSetList == null){
            $this->inputSetList = $this->getServiceLocator()->get('Model\InputSetList');
        }
...
//**********************************************************************************************************
    ...
    public function indexAction()
    {
        ...
        if($request->isPost()){
            ...
            $paginator = $this->getInputSetList()->search($pageNo, $request->getPost());
        }else{
            //index without search
            $paginator = $this->getInputSetList()->fetchAll($pageNo);
        }
        ...
        //sub view search_bar
        $searchItems = array('input_set_cd','input_set_name');
        $searchButton  = array(
            'url' => array(
                'route' => 'master/default',
                'controller' => 'inputSetLists',
                'action' => 'index' 
            ),
            'name' => '検索',
        );
    ...
    {   
        ...
        $data = $this->getInputSetList()->get($id);
        ...
        $detailList = array(
            '入力値制御コード' => 'input_set_cd',
            '入力値制御名' => 'input_set_name', 
            '最小値' => 'max_value', 
            '最大値' => 'min_value',
            '実施値' => 'imp_value',
            '実施作業' => 'imp_work',
    ...
    public function editAction()
    {
        ...
        if($status == '保存'){
            ...
            $result = $this->getInputSetList()->save($this->getRequest()->getPost(), $this->auth);

            if($result){
                if($id){
                    $this->flashMessenger()->addMessage($request->getPost()['input_set_name'].'の編集が完了しました。');
                }else{
                    $this->flashMessenger()->addMessage($request->getPost()['input_set_name'].'の新規追加が完了しました。');
                }
        ...
        if($status == '確認'){
            $form->setData($request->getPost());
            $form->setInputFilter($this->getInputSetList()->getInputFilter());
        ...
        if($status == '編集'){
            ...
            try {
                $inputSetList = $this->getInputSetList()->get($id);
            }
            catch (\Exception $ex) {
                return $this->redirect()->toRoute('master/default',array(
                    'controller' => 'inputSetLists',
...
/*
    public function confirmAction()
    {
        ...
        $itemList = array('input_set_cd','input_set_name','inputSetList_postcode','inputSetList_add1','inputSetList_add2','inputSetList_tel','inputSetList_fax');
    ...
    public function saveAction(){
        ...
        if($result){
            if($id){
                $this->flashMessenger()->addMessage($inputSetList['input_set_name'].'の編集が完了しました。');
            }else{
                $this->flashMessenger()->addMessage($inputSetList['input_set_name'].'の新規追加が完了しました。');
            }
    ...
    public function addAction()
    {
        ...
        if($request->isPost()){
            ...
            if($form->isValid()){
                ...
                $result = $this->getInputSetList()->save($form->getData(), $this->auth);

                if($result){
                    $this->flashMessenger()->addMessage($request->getPost()['input_set_name'].'の新規登録が完了しました。');
                }
