...
class BranchsController extends ControllerBasicWithAuthentication
{
    protected $formName = 'BranchForm';

    public $branch;

    public function getBranch(){
        if($this->branch == null){
            $this->branch = $this->getServiceLocator()->get('Model\Branch');
        }
        return $this->branch;
    }
...
//**********************************************************************************************************
    ...
    public function indexAction()
    {
        ...
        if($request->isPost()){
            ...
            $paginator = $this->getBranch()->search($pageNo, $request->getPost());
        }else{
            //index without search
            $paginator = $this->getBranch()->fetchAll($pageNo);
        }
        ...
        //sub view search_bar
        $searchItems = array('branch_cd','branch_name');
        $searchButton  = array(
            'url' => array(
    ...
    public function editAction()
    {
        ...
        if($status == '保存'){
            ...
            if($result){
                if($id){
                    $this->flashMessenger()->addMessage($request->getPost()['branch_name'].'の編集が完了しました。');
                }else{
                    $this->flashMessenger()->addMessage($request->getPost()['branch_name'].'の新規追加が完了しました。');
                }
        ...
        //sub view edit_list
        $itemList = array('branch_cd','branch_name','branch_kana','branch_tel','branch_postcode','branch_add1','branch_add2','branch_fax'
            ,'area_id','app_tel','hotline_tel','contact_tel');
