...
class TmpInputHeadersController extends ControllerBasicWithAuthentication
{
    protected $formName = 'TmpInputHeaderForm';

    public $tmpInputHd;

    public function getTmpInputHeader(){
        if($this->tmpInputHd == null){
            $this->tmpInputHd = $this->getServiceLocator()->get('Model\TmpInputHeader');
        }
        return $this->tmpInputHd;
    }
...
//**********************************************************************************************************
    ...
    public function indexAction()
    {
        ...
        if($request->isPost()){
            ...
            $paginator = $this->getTmpInputHeader()->search($pageNo, $request->getPost());
        }else{
            //index without search
            $paginator = $this->getTmpInputHeader()->fetchAll($pageNo);
        }
        ...
        //sub view search_bar
        $searchItems = array('tmp_input_hd_cd','tmp_input_hd_name');
        $searchButton  = array(
            'url' => array(
                'route' => 'master/default',
                'controller' => 'tmpInputHeaders',
                'action' => 'index' 
            ),
            'name' => '検索',
        );
    ...
    {   
        ...
        $data = $this->getTmpInputHeader()->get($id);
        ...
        $detailList = array(
            'テンプレートフォーマットデータコード' => 'tmp_input_hd_cd', 
            '入力項目名' => 'tmp_input_hd_name', 
            '入力形式ID' => 'input_type_id',
        );
    ...
    public function editAction()
    {
        ...
        if($status == '保存'){
            ...
            $result = $this->getTmpInputHeader()->save($this->getRequest()->getPost(), $this->auth);

            if($result){
                if($id){
                    $this->flashMessenger()->addMessage($request->getPost()['tmp_input_hd_name'].'の編集が完了しました。');
                }else{
                    $this->flashMessenger()->addMessage($request->getPost()['tmp_input_hd_name'].'の新規追加が完了しました。');
                }
        ...
        if($status == '確認'){
            $form->setData($request->getPost());
            $form->setInputFilter($this->getTmpInputHeader()->getInputFilter());
        ...
        if($status == '編集'){
            ...
            try {
                $tmpInputHd = $this->getTmpInputHeader()->get($id);
            }
            catch (\Exception $ex) {
                return $this->redirect()->toRoute('master/default',array(
                    'controller' => 'tmpInputHeaders',
...
/*
    public function confirmAction()
    {
        ...
        $itemList = array('tmp_input_hd_cd','tmp_input_hd_name','tmpInputHd_postcode','tmpInputHd_add1','tmpInputHd_add2','tmpInputHd_tel','tmpInputHd_fax');
    ...
    public function saveAction(){
        ...
        if($result){
            if($id){
                $this->flashMessenger()->addMessage($tmpInputHd['tmp_input_hd_name'].'の編集が完了しました。');
            }else{
                $this->flashMessenger()->addMessage($tmpInputHd['tmp_input_hd_name'].'の新規追加が完了しました。');
            }
    ...
    public function addAction()
    {
        ...
        if($request->isPost()){
            ...
            if($form->isValid()){
                ...
                $result = $this->getTmpInputHeader()->save($form->getData(), $this->auth);

                if($result){
                    $this->flashMessenger()->addMessage($request->getPost()['tmp_input_hd_name'].'の新規登録が完了しました。');
                }
