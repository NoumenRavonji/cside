...
class TmpFmtDatasController extends ControllerBasicWithAuthentication
{
    protected $formName = 'TmpFmtDataForm';

    public $tmpFmtData;

    public function getTmpFmtData(){
        if($this->tmpFmtData == null){
            $this->tmpFmtData = $this->getServiceLocator()->get('Model\TmpFmtData');
        }
        return $this->tmpFmtData;
    }
...
//**********************************************************************************************************
    ...
    public function indexAction()
    {
        ...
        if($request->isPost()){
            ...
            $paginator = $this->getTmpFmtData()->search($pageNo, $request->getPost());
        }else{
            //index without search
            $paginator = $this->getTmpFmtData()->fetchAll($pageNo);
        }
        ...
        //sub view search_bar
        $searchItems = array('tmp_fmt_data_cd','tmp_fmt_data_name');
        $searchButton  = array(
            'url' => array(
                'route' => 'master/default',
                'controller' => 'tmpFmtDatas',
                'action' => 'index' 
            ),
            'name' => '検索',
        );
    ...
    {   
        ...
        $data = $this->getTmpFmtData()->get($id);
        ...
        $detailList = array(
            'テンプレートフォーマットID' => 'tmp_fmt_id',
            'テンプレートフォーマットデータコード' => 'tmp_fmt_data_cd', 
            '入力項目名' => 'tmp_fmt_data_name', 
            '入力形式ID' => 'input_type_id', 
            '選択入力ID' => 'tmp_imput_id',
            '自動取込対象カラム' => 'set_mst_shop',
            '入力画面での表示場所' => 'disp_num',
        );
    ...
    public function editAction()
    {
        ...
        if($status == '保存'){
            ...
            $result = $this->getTmpFmtData()->save($this->getRequest()->getPost(), $this->auth);

            if($result){
                if($id){
                    $this->flashMessenger()->addMessage($request->getPost()['tmp_fmt_data_name'].'の編集が完了しました。');
                }else{
                    $this->flashMessenger()->addMessage($request->getPost()['tmp_fmt_data_name'].'の新規追加が完了しました。');
                }
        ...
        if($status == '確認'){
            $form->setData($request->getPost());
            $form->setInputFilter($this->getTmpFmtData()->getInputFilter());
        ...
        if($status == '編集'){
            ...
            try {
                $tmpFmtData = $this->getTmpFmtData()->get($id);
            }
            catch (\Exception $ex) {
                return $this->redirect()->toRoute('master/default',array(
                    'controller' => 'tmpFmtDatas',
...
/*
    public function confirmAction()
    {
        ...
        $itemList = array('tmp_fmt_data_cd','tmp_fmt_data_name','tmpFmtData_postcode','tmpFmtData_add1','tmpFmtData_add2','tmpFmtData_tel','tmpFmtData_fax');
    ...
    public function saveAction(){
        ...
        if($result){
            if($id){
                $this->flashMessenger()->addMessage($tmpFmtData['tmp_fmt_data_name'].'の編集が完了しました。');
            }else{
                $this->flashMessenger()->addMessage($tmpFmtData['tmp_fmt_data_name'].'の新規追加が完了しました。');
            }
    ...
    public function addAction()
    {
        ...
        if($request->isPost()){
            ...
            if($form->isValid()){
                ...
                $result = $this->getTmpFmtData()->save($form->getData(), $this->auth);

                if($result){
                    $this->flashMessenger()->addMessage($request->getPost()['tmp_fmt_data_name'].'の新規登録が完了しました。');
                }
