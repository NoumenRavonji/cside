...
class TmpInputDatasController extends ControllerBasicWithAuthentication
{
    protected $formName = 'TmpInputDataForm';

    public $tmpInputData;

    public function getTmpInputData(){
        if($this->tmpInputData == null){
            $this->tmpInputData = $this->getServiceLocator()->get('Model\TmpInputData');
        }
        return $this->tmpInputData;
    }
...
//**********************************************************************************************************
    ...
    public function indexAction()
    {
        ...
        if($request->isPost()){
            ...
            $paginator = $this->getTmpInputData()->search($pageNo, $request->getPost());
        }else{
            //index without search
            $paginator = $this->getTmpInputData()->fetchAll($pageNo);
        }
        ...
        //sub view search_bar
        $searchItems = array('tmp_input_id','tmp_input_datas_name');
        $searchButton  = array(
            'url' => array(
                'route' => 'master/default',
                'controller' => 'tmpInputDatas',
                'action' => 'index' 
            ),
            'name' => '検索',
        );
    ...
    {   
        ...
        $data = $this->getTmpInputData()->get($id);
        ...
        $detailList = array(
            '選択入力ID' => 'tmp_input_id',
            '選択入力表示名' => 'tmp_input_datas_name', 
            '選択入力登録名' => 'tmp_input_datas_disp', 
            '表示画像' => 'tmp_input_datas_img',
        );
    ...
    public function editAction()
    {
        ...
        if($status == '保存'){
            ...
            $result = $this->getTmpInputData()->save($this->getRequest()->getPost(), $this->auth);

            if($result){
                if($id){
                    $this->flashMessenger()->addMessage($request->getPost()['tmp_input_datas_name'].'の編集が完了しました。');
                }else{
                    $this->flashMessenger()->addMessage($request->getPost()['tmp_input_datas_name'].'の新規追加が完了しました。');
                }
        ...
        if($status == '確認'){
            $form->setData($request->getPost());
            $form->setInputFilter($this->getTmpInputData()->getInputFilter());
        ...
        if($status == '編集'){
            ...
            try {
                $tmpInputData = $this->getTmpInputData()->get($id);
            }
            catch (\Exception $ex) {
                return $this->redirect()->toRoute('master/default',array(
                    'controller' => 'tmpInputDatas',
        ...
        $itemList = array(
           'tmp_input_id',
            'tmp_input_datas_name', 
            'tmp_input_datas_disp', 
            'tmp_input_datas_img',);
...
/*
    public function confirmAction()
    {
        ...
        $itemList = array('tmp_input_id','tmp_input_datas_name','tmpInputData_postcode','tmpInputData_add1','tmpInputData_add2','tmpInputData_tel','tmpInputData_fax');
    ...
    public function saveAction(){
        ...
        if($result){
            if($id){
                $this->flashMessenger()->addMessage($tmpInputData['tmp_input_datas_name'].'の編集が完了しました。');
            }else{
                $this->flashMessenger()->addMessage($tmpInputData['tmp_input_datas_name'].'の新規追加が完了しました。');
            }
    ...
    public function addAction()
    {
        ...
        if($request->isPost()){
            ...
            if($form->isValid()){
                ...
                $result = $this->getTmpInputData()->save($form->getData(), $this->auth);

                if($result){
                    $this->flashMessenger()->addMessage($request->getPost()['tmp_input_datas_name'].'の新規登録が完了しました。');
                }
